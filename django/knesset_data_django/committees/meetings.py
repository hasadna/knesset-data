...
def reparse_protocol(committee_meeting, redownload=True, mks=None, mk_names=None):
    if redownload: committee_meeting.redownload_protocol()
    if committee_meeting.committee.type == 'plenum':
        parse_for_existing_meeting(committee_meeting)
    else:
        committee_meeting.create_protocol_parts(delete_existing=True)
        committee_meeting.find_attending_members(mks, mk_names)
